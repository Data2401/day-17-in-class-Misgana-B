...
heights %>%
  filter(sex == "Female") %>%
  summarize(tallest = max(height))

heights %>%
  filter(sex == "Male") %>%
  summarize(shortest = min(height))
...
```{r}
ggplot(votedata %>% filter(year == 2000), aes(x = reorder(state, demVote), y = demVote, fill = south)) + 
  geom_col() + 
  coord_flip()
...
ggplot(votedata %>% filter(year == 2000), aes(x = reorder(state, demVote), y = demVote, fill = south)) + 
  geom_col() + 
  coord_flip() + 
  labs(title = "Democratic Vote 2000", 
       subtitle = "Percentage of Democratic Votes", 
       x = "State", y = "Democratic Vote ")
...
```{r}
ggplot(votedata %>% filter(year %in% c(2008, 2016)), aes(x = reorder(state, demVote), y = demVote, fill = south)) + 
  geom_col() + 
  coord_flip() + 
  facet_wrap(~ year)
...
ggplot(votedata %>% filter(year == 2000), aes(x = reorder(state, demVote), y = demVote, fill = south)) + 
  ...
  labs(title = "Democratic vote 2000)", 
       subtitle = "Percentage  Votes", 
       x = "State", y = "Democratic Vote Percentage")
...
penguins %>% group_by(island) %>% summarize(avg_mass = mean(body_mass_g))


penguins %>% group_by(sex) %>% summarize(avg_bill = mean(bill_length_mm))
...
ggplot(penguins, aes(flipper_length_mm, fill = sex)) + geom_density(alpha = 0.5)
